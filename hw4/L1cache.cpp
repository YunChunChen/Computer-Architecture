...
int L1cache::getfromCache(const int address) {
    ...
    for (int i = 0 ; i < way_number ; i++) {
        if (cache[row+i][0] == 1 && cache[row+i][3] == tag) {
            ...
            cache[row+i][2] = way_number; // set release bit to 0
            for (int j = 0 ; j < way_number ; j++)
                if (j != i)
                    cache[row+j][2] -= 1; // increment release bit by 1
    ...
    if (cache[_row][1] == 1) // dirty bit is one, write back to memory
        mem->writetoMem(cache[_row][3], cache[_row]+4); // write to memory
    ...
    cache[_row][2] = way_number; // set release bit to 0
    cache[_row][3] = tag; // update tag
    for (int i = 0 ; i < way_number ; i++)
        if (i != index)
            cache[row+i][2] -= 1; // increment release bit by 1
...
void L1cache::writetoCache(const int address,const int indata) {
    ...
    if (cache[_row][1] == 1) // dirty bit is 1
        mem->writetoMem(cache[_row][3], cache[_row]+4); // write to memory
    int* pointer = mem->getfromMem(tag); // get from memory
    for (int j = 0 ; j < 4 ; j++)
...
int L1cache::getReadHit(void) { return L1readhit; }
int L1cache::getReadMiss(void) { return L1readmiss; }
int L1cache::getWriteHit(void) { return L1writehit; }
int L1cache::getWriteMiss(void) { return L1writemiss; }
int L1cache::getHit(void) { return L1readhit + L1writehit; }
int L1cache::getMiss(void) { return L1readmiss + L1writemiss; }